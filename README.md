...
```sh
./target/release/chameleon-node-devnet -h
```
...
```sh
./target/release/chameleon-node-devnet purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/chameleon-node-devnet -ldebug --dev
```
