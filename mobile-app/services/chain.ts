...
class ChainService {
  ...
  public async getBalance(address: string): Promise<BalanceInfo> {
    console.log('[ChainService] getBalance called for:', address);
    
    const api = apiService.getApi();
    if (!api) {
      console.log('[ChainService] getBalance - API not connected');
      throw new Error('API not connected');
    }

    console.log('[ChainService] Querying system.account...');
    ...
    console.log('[ChainService] Balance result - free:', free, 'reserved:', reserved);
