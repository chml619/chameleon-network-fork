...
class TransactionService {
   ...
   * Fixed: Added pending status callback, better error handling
   */
  public async sendTransactionWithMonitoring(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Flag to track if we've resolved/rejected
      let isComplete = false;
      
      tx.signAndSend(from, (result: ISubmittableResult) => {
        try {
          const hash = result.txHash.toHex();
          console.log('[Transaction] Status update:', result.status.type, 'hash:', hash);
          
          // Send pending status on first callback (when tx is broadcast)
          ...
          if (result.status.isInBlock) {
            ...
            if (isComplete) return; // Prevent duplicate resolution
            isComplete = true;
            ...
            api.rpc.chain.getHeader(blockHash).then((header) => {
              ...
              if (failed) {
                ...
                if (errorEvent) {
                  ...
                  if ((dispatchError as any).isModule) {
                    try {
                      ...
                      console.error('[Transaction] Error decoding failure:', e);
                    }
                  }
                }
                ...
                console.log('[Transaction] Success! Block:', blockNumber);
                onStatusUpdate({
                  hash,
                  status: 'finalized',
                  blockHash,
              ...
              console.error('[Transaction] Error getting header:', headerError);
              // Still resolve but without block number
              onStatusUpdate({
                hash,
                status: 'finalized',
                blockHash,
          ...
          console.error('[Transaction] Callback error:', callbackError);
          if (!isComplete) {
            isComplete = true;
        ...
        console.error('[Transaction] Error in signAndSend:', error);
        if (!isComplete) {
          isComplete = true;
          reject(error);
        }
